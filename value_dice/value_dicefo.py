...
class ValueDICEfO(object):
  """Class that implements DualDICE training. Modifications: mask out expert actions, use pseudo ones generated
  by a inverse action model."""

  def __init__(self, state_dim, action_dim, log_interval, nu_lr, actor_lr,
               alpha_init, hidden_size, inverse_lr=1e-3):
    ...
    Args:
      ...
      inverse_lr: inverse learning rate
    ...
    self.avg_inverse_loss = tf.keras.metrics.Mean('inverse loss', dtype=tf.float32)
  ...
  def update(self,
    ...
    (expert_states, _, expert_next_states) = expert_dataset_iter.get_next()
    ...
    inverse_loss = self.fit_inverse(rb_states, rb_actions, rb_next_states)
    self.avg_inverse_loss(inverse_loss)

    expert_actions = self.inverse(expert_states, expert_next_states)
    ...
    if tf.equal(self.nu_optimizer.iterations % self.log_interval, 0):
      tf.summary.scalar(
          'train inverse loss',
          self.avg_inverse_loss.result(),
          step=self.inverse_optimizer.iterations)
      keras_utils.my_reset_states(self.avg_inverse_loss)
